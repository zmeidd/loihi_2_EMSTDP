...
class loihi2_net(multipattern_learning):
    ...
    def test_non_conv_loihi(self,dataset, w_h,w_o):
        ...
        if not self.loihi:
            ...
            start = time.time()
            print("start testing bias input: ", )
            ...
            c = LIFReset(shape=(c_features,),vth = 1000,bias_mant= 0,du=4095,dv=0,reset_interval = self.time_steps)
            ...
            # nc_domain = SyncDomain("nc", LoihiProtocol(), [pattern_pre,con1,a,b,con2,c])
    ...
    def test_non_conv_loihi_fast_io(self,dataset,w_h=[],w_o=[], profiling = False):
        ...
        if profiling:
            ...
            profiler.energy_probe(num_steps=num_steps*num_samples)
...
net = loihi2_net([200,100,10],time_steps = 32, conv =False, fast_io= False)
dataset = [data,label]
...
net.test_non_conv_loihi_fast_io(dataset,w_h,w_o)
# print("done testing!")
