...
train_size = 50000                          # verification period
...
for ep in range(epochs):
    ...
    for i in trange(int (train_size / batch_size)):
        if ((i + 1) * batch_size % ver_period == 0):  # 5000
            ...
            for i2 in range(int(test_size / tbs)):  # train_size
                ...
                pred[i2 * tbs:(i2 + 1) * tbs] ,_= snn_network.Test(spikes2.astype(float), tbs)
    ...
    np.save("w_h_mnist.npy", snn_network.w_h)
    np.save("w_o_mnist.npy",snn_network.w_o)