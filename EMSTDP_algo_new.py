...
class Network(object):
    ...
    def Train(self, input_spikes, label, bs):
        ...
        for t in range(0, self.T):
            # first phase
            U_h[0][t, :, :] = oe.contract("Bi,ij->Bj", input_spikes[t, :, :], self.w_h[0])  + U_h[0][t - 1, :, :] + self.threshold_h[0]*self.bias
            hidden_spikes[0][t, :, :] = U_h[0][t, :, :] > self.threshold_h[0]
            U_h[0][t, hidden_spikes[0][t, :, :]] = 0
            U_h[0][t, :, droph[0]] = 0

            for h in range(1, n_hidden):
                U_h[h][t, :, :] = oe.contract("Bi,ij->Bj", hidden_spikes[h - 1][t, :, :], self.self.w_h[h]) + U_h[h][t - 1, :, :] + self.threshold_h[h] * self.bias
                hidden_spikes[h][t, :, :] = U_h[h][t, :, :] > self.threshold_h[h]
                U_h[h][t, hidden_spikes[h][t, :, :]] = 0
                U_h[h][t, :, droph[h]] = 0

            U_o[t, :, :] = oe.contract("Bi,ij->Bj", hidden_spikes[n_hidden - 1][t, :, :], self.w_o) + U_o[t - 1, :, :] + self.threshold_o * self.bias
            ...
            if (t > (self.twin)):
                ...
                U_o[t, :, :] += self.threshold_o * sdelta[t - 1, :, :]
                ...
                if t>(self.twin * 2):
                    ...
                    self.w_o += np.multiply(tpp, newlr)/2
                    if self.clp:
                ...
                if t>(self.twin*2):
                    ...
                    self.w_h[0] += np.multiply(tpp, newlr)/2
                    if self.clp:
    ...
    def Test(self, input_spikes, bs):
        ...
        for t in range(1, self.T):
            ...
            output_spikes[t, :, :] = U_o[t, :, :] >= self.threshold_o
            U_o[t, output_spikes[t, :, :]] = 0
        print("out threshold !",  self.threshold_o)
        return np.argmax(np.sum(output_spikes[:,:,:], axis=0), axis=-1), np.sum(hidden_spikes[0][:,:,:], axis=0)/self.T
