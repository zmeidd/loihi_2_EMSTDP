...
class loihi2_net(multipattern_learning):
        ...
        if not self.fast_io:
            if not self.conv:
                ...
                for i in range(len(data)):
                    ...
                    b = b.astype(int)
                    #run intervals
                    self.lif_input.run(condition=RunSteps(num_steps=self.time_steps//2), run_cfg=  Loihi2HwCfg())
                    self.lif_input.bias_mant.set((65*inputs).astype(int))
                    self.label.bias_mant.set(b)
                    self.lif_input.run(condition=RunSteps(num_steps=self.time_steps//2), run_cfg=  Loihi2HwCfg())
    ...
    def test_loihi(self,dataset, w_h=[],w_o=[]):
        ...
        pattern_pre = LIF(shape=(features,),vth = 100,bias_mant= 0,du=4095,dv=0)
        #w_h = w_h[0]
        if len(w_h) ==0:
        ...
        if not self.loihi:
            ...
            # assist = LIF(shape=(c_features,),vth = 16,bias_mant= 0,du=4095,dv=0)
            ...
            for i in range(num_samples):
                pattern_pre.bias_mant.set((65*10*inputs[i]).astype(int))
                # pattern_pre.bias_mant.set(inputs[i])
                # self.assist_test(assist,demo_assist[i])
